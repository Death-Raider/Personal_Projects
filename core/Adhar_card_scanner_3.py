...
pathImage = "path/to/img"
paths="http://192.168.119.129:8080/video"
...
while True:
    ...
    key=cv2.waitKey(1)
    if key == ord('s'):
        scanpath=f"path/to/Scanned_image"
        imgWarpColored=cv2.resize(imgWarpColored,(1080,720))
        # data = pytesseract.image_to_string(imgWarpColored, config=MODEL_CONFIG) + ' ' # TODO: FIX and utilize pytesseract for better performance
        # print(data)
        if imgWarpColored is not None:
            cv2.imwrite(scanpath+ "\\" + str(count) + ".jpg", imgWarpColored)
        utils.display_img(imgWarpColored)
        info=utils.extractDataFromIdCard(imgWarpColored)
